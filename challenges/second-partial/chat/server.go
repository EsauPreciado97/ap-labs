			...
			case "/kick":
				if len(incoming) > 1 {
					if isAdmin(ch) {
						...
						if foundUser {
							messages <- "irc-server > " + incoming[1] + " was kicked by the channel ADMIN"
						} else {
							ch <- "irc-server > User not found"
						}
					} else {
						ch <- "irc-server > Permission Denied: User is not ADMIN"
					}
				} else {
					ch <- "irc-server > ERROR: Missing argument. Correct use: /kick [user]"
				}
			default:
				if notKicked(ch) {
